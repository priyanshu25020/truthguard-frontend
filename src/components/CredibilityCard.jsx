...
const CredibilityCard = ({ score = 0 }) => {
  // Score (0-100) ko 5 stars me convert karna
  const activeStars = Math.round((score / 100) * 5);
  
  // Score ke hisaab se label decide karna
  let label = "Awaiting Analysis";
  if (score > 0) {
    if (score >= 75) label = "High Credibility";
    else if (score >= 40) label = "Medium Credibility";
    ...
    <div className="bg-gradient-to-br from-blue-900/30 to-[#05040a] border border-blue-500/20 rounded-[20px] p-6 shadow-[inset_0_0_30px_rgba(59,130,246,0.08)] hover:border-blue-500/40 transition-all duration-300 h-full">
      ...
      <div className="flex gap-1.5 mb-4">
        {[...Array(5)].map((_, i) => (
          <svg 
            key={i} 
            className={`w-5 h-5 transition-colors duration-500 ${i < activeStars ? 'text-yellow-500 drop-shadow-[0_0_8px_rgba(234,179,8,0.6)]' : 'text-gray-700/80'}`} 
            fill="currentColor" 
            viewBox="0 0 20 20"
          >
      ...
      <p className="text-[13px] text-[#94a3b8] font-semibold">{label}</p>
    </div>
  );
};
